...
def build_tree(path, parent=None):
    ...
    if os.path.isdir(path):
        ...
        for entry in os.listdir(path):
            ...
            # Recursively build the tree, skipping .git directories
            child_node = build_tree(full_path, parent=node)
            if child_node:  # Only add the child if it's not None (skipping .git)
